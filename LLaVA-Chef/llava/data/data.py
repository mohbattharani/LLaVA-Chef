...
class RecipeM ():
    ...
    def get_sample (self, idx):
        ...
        if ("image" in sample.keys()):
            image = random.sample (sample['image'], 1)[0]
            sample['image_path']= os.path.join (self.image_dir, image)
    ...
    def conversation (self, idx):
        ...
        final_keys = self.prompt_input_mapping (selected_prompt_keys)
        target = self.targets [selected_prompt_keys]
        
        if ("<title>" in target):
            ingredients = random.sample (sample['ingredients'], random.choice (range(len(sample['ingredients'])//2, len(sample['ingredients']))))
        else:
        ...
        if ("title" in final_keys):
            title = sample['title']
        else:
            title = random.choice (["food", "dish"])

        if ("<name>" in q):
            q = q.replace("<name>", title) 
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        sources = self.recipe.conversation (i)
